...
class VGG(nn.Module):
    ...
    def __init__(self, features, classes=10, precision=-1, init_weights=True):
        ...
        if (precision < 0):
            ...
            self.classifier = zs_quantized_ops.nnLinearSymQuant_op(512, classes, precision, fc_weight_clamp)
...
def vgg(cfg,input_channels,classes,batch_norm,precision):
    model = VGG(make_layers(cfgs[cfg], in_channels = input_channels, batch_norm=batch_norm, precision=precision), classes, precision, True )
    return model
