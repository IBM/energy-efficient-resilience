...
def applyBitAttack(clean, perturbed):
    return (
        clean
        ^ (2 ** torch.floor(torch.log2(clean ^ perturbed))).to(torch.uint8)
    ).to(torch.uint8)
...
def bitErrorAttack(
    ...
    mask[topK] = 1
    changeParamTmp = allParamPerturb + alpha * (
        mask * allGrads
    )  # Apply gradients ascent on perturbed weights, and set it temp variable.
    changeParamTmp = torch.clamp(
        changeParamTmp, min=-128 * allDeleta, max=127 * allDeleta
    )  # Clamp the perturbed weights into correct range.
    cleanParam_q = torch.round(
        (allParamClean / allDeleta)
    )  # Quantized the weights from the clean model by using symmetric-signed.
    cleanParam_q = cleanParam_q.to(torch.int8)
    changeParam_q = torch.round(
        (changeParamTmp / allDeleta)
    )  # Quantized the weights of the perturbed model by using symmetric-signed.
    ...
    bit_tmp = applyBitAttack(
        cleanParam_q.to(torch.uint8), changeParam_q.to(torch.uint8)
    )
    bit_tmp = bit_tmp.to(torch.int8)
    allParamPerturb = (
        bit_tmp * allDeleta
    )  # Dequantized the perturbed weights and set the results to perturbed models.
...
def pgd(model_origin, model_p, alpha, precision, device):
    ...
    storeParamNum = (
        []
    )  # Store how many weights in each layer. This is for reshaping the weights.

    # Store the flatten weights and gradient first:
    for (name_origin, param_origin), (name_p, param_p) in zip(
        model_origin.named_parameters(), model_p.named_parameters()
    ):
        if not "bn" in name_p:
    ...
    for name_p, param_p in model_p.named_parameters():
        if not "bn" in name_p:
            param_p.data = torch.reshape(
                allParam[weightNum : weightNum + storeParamNum[countNum]],
                param_p.shape,
            )
...
def transform_train(
    ...
    print(
        "========== Check setting: Epoch: {}, Batch_size: {}, Lambda: {}, BitErrorRate: {}, G: {} ==========".format(
            cfg.epochs, cfg.batch_size, lb, ber, cfg.G
        )
    )
    print(
    ...
    for epoch in range(cfg.epochs):
        ...
        if (epoch + 1) % 50 == 0 or (epoch + 1) == cfg.epochs:
            ...
            torch.save(
                Gen.state_dict(),
                cfg.save_dir
                + "Adversarial_GeneratorV1_{}_arch_{}_LR_a{}_p{}_E_{}_PGD_{}_ber_{}_lb_{}_NOWE_{}_bit_10000.pt".format(
                    dataset,
                    arch,
                    cfg.alpha,
                    cfg.learning_rate,
    ...
    plt.savefig(
        cfg.save_dir
        + "Adversarial_GeneratorV1_{}_arch_{}_LR_a{}_p{}_E_{}_PGD_{}_ber_{}_lb_{}_NOWE_{}_bit_10000_Learning_Curve.jpg".format(
            dataset,
            arch,
            cfg.alpha,
            cfg.learning_rate,
