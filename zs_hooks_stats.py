...
class DataLogger:
    ...
    def __init__(self, num_batches, batch_size):
        print('init logger with number of batches %d, batch size %d'%(num_batches,batch_size))
    ...
    def update(self, model_outputs):
        ...
        self.confidences[self.batch_id,0:self.batch_size] = confidences.values.detach().cpu().numpy()
        self.predictions[self.batch_id,0:self.batch_size] = predictions.detach().cpu().numpy()
        self.logits[self.batch_id,0:self.batch_size] = logits.detach().cpu().numpy()
        self.batch_id +=1
...
def plot(data):
    ...
    #            print(weights.shape)
    #            print('Max %.3f Min %.4f proportion of Non zero elements %.4f'%(np.max(weights), np.min(weights), np.count_nonzero(weights)/np.prod(weights.shape)))
...
def resnet_layer_1_stats(self, input, output):
    ...
    non_zeros=torch.numel(torch.nonzero(output.data))/4   ## torch.nonzero returns the non-zero element indices
...
def resnet_print_stats():
    print('Proportion of non-zero elements in layer 1 %.3f'%(resnet_layer_1/600))
    print('Proportion of non-zero elements in layer 2 %.3f'%(resnet_layer_2/800))
    print('Proportion of non-zero elements in layer 3 %.3f'%(resnet_layer_3/1200))
    print('Proportion of non-zero elements in layer 4 %.3f'%(resnet_layer_4/600))
 #  print('Proportion of non-zero elements in layer 5 %.3f'%(resnet_layer_5/100))
