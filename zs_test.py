...
def init_models(arch, bit_error_rate, precision, position, faulty_layers, checkpoint_path, device):
    ...
    if (not cfg.faulty_layers):
        ...
        """
        rf = randomfault.RandomFaultModel(bit_error_rate, precision, position, 0)
        ...
        if arch == 'vgg11':
          model  = vggf('A',in_channels, 10, True, precision, bit_error_rate, position, BitErrorMap0, BitErrorMap1, faulty_layers)
        elif arch == 'vgg16':
          model  = vggf('D',in_channels, 10, True, precision, bit_error_rate, position, BitErrorMap0, BitErrorMap1, faulty_layers)
        elif arch == 'resnet18':
          model = resnetf('resnet18', 10, precision, bit_error_rate, position, BitErrorMap0, BitErrorMap1, faulty_layers) 
        elif arch == 'resnet34':
          model = resnetf('resnet34', 10, precision, bit_error_rate, position, BitErrorMap0, BitErrorMap1, faulty_layers) 
        else:
          model = lenetf(in_channels,10,precision, bit_error_rate, position, BitErrorMap0, BitErrorMap1, faulty_layers)
...
def inference(testloader, arch, dataset, ber, precision, position, checkpoint_path, faulty_layers, device):
    model = init_models(arch, ber, precision, -1, faulty_layers, checkpoint_path, device)
