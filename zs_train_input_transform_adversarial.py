...
def diff_in_weight(model_orig, model_p):
    ...
    model_stateDict = model_orig.state_dict()
    model_perturbed_stateDict = model_p.state_dict()
    for(old_k, old_w), (new_k, new_w) in zip(model_stateDict.items(), model_perturbed_stateDict.items()):
        if len(old_w.size()) <= 1:
            continue
        if 'weight' in old_k:
            diff_w = new_w - old_w
...
def add_into_weights(model_p, diff, coeff=1.0):
...
def pgd(model_orig, model_p, epsilon, alpha):
    for (name_orig, param_orig), (name_p, param_p) in zip(model_orig.named_parameters(), model_p.named_parameters()):
        if param_p.requires_grad == True:
            with torch.no_grad():
                delta = quantization(param_orig)
                param_p.add_(alpha * param_p.grad.sign())
                param_p.data.clamp_(min=param_orig-(epsilon*delta), max=param_orig+(epsilon*delta))
...
def transform_train(
    ...
    torch.save({'Reprogrammed Perturbation': Pg.P},
        cfg.save_dir + 'adversarial_arch_{}_LR_a{}_p{}_E_{}_PGD_{}_ber_{}_lb_{}.pt'.format(arch, cfg.learning_rate_adversarial, cfg.learning_rate, cfg.epochs, cfg.PGD_STEP, ber, lb))