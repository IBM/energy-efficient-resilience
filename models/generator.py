...
class DoubleConvQ(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, precision, mid_channels=None
    ):
        super().__init__()
        if precision > 0:
            if not mid_channels:
...
class DownQ(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, precision):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2), DoubleConvQ(in_channels, out_channels, precision)
        )
    ...
    def __init__(self, in_channels, out_channels, precision, bilinear=True):
        ...
        if precision > 0:
            if bilinear:
                ...
                self.conv = DoubleConvQ(
                    in_channels, out_channels, in_channels // 2, precision
                )
            else:
