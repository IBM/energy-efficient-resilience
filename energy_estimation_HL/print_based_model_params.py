...
def collect_activation_sizes(self, input, output):
    ...
    if 'Conv2d' in self.__class__.__name__ or 'Linear' in self.__class__.__name__:
        ...
        if 'Conv2d' in self.__class__.__name__:
            ...
            m_strides[layer_counter] = 1 # Must modify by myself
        elif 'Linear' in self.__class__.__name__:
            input_w[layer_counter] = 1
            input_h[layer_counter] = 1
            input_c[layer_counter] = i_shape[1]
            filter_w[layer_counter] = 1
            filter_h[layer_counter] = 1
            filter_c[layer_counter] = f_shape[0]
...
def main():
    ...
    in_channels = 3
    classes = 10
    # model = vggf("A", in_channels, classes, True, 8, 0, 0, [])
    # model = vggf("D", in_channels, classes, True, 8, 0, 0, [])
    model = vggf("E", in_channels, classes, True, 8, 0, 0, [])
    # model = resnetf("resnet18", classes, 8, 0, 0, [])
    # model = resnetf("resnet50", classes, 8, 0, 0, [])
    print(model)
    model.eval()
    model.to(device)
    hooks = {}
    for step, (name, module) in enumerate(model.named_modules()):
