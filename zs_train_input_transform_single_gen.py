...
def compute_diversity_loss(img_origin, img_random, gen, device):
    ...
    for img_o, img_r, pertrub_o, perturb_r in zip(
        img_origin, img_random, perturb_origin, perturb_random
    ):
        diff_img_norm = torch.linalg.norm(
            torch.reshape(img_o - img_r, (-1,)), dim=0, ord=2
        )
        diff_p_norm = torch.linalg.norm(
            torch.reshape(pertrub_o - perturb_r, (-1,)), dim=0, ord=2
        )
...
def transform_train(
    ...
    print(
        "========== Start training the parameter of the input transform by using one specific perturbed model =========="
    )
    ...
    for epoch in range(cfg.epochs):
        ...
        if (epoch + 1) % 50 == 0 or (epoch + 1) == cfg.epochs:
            ...
            torch.save(
                Gen.state_dict(),
                cfg.save_dir
                + "Single_GeneratorV1_{}_arch_{}_LR{}_E_{}_ber_{}_lb_{}_BS_{}_NOWE_{}_step500.pt".format(
                    dataset,
                    arch,
                    cfg.learning_rate,
                    cfg.epochs,
            ...
            plt.savefig(
                cfg.save_dir
                + "Single_GeneratorV1_{}_arch_{}_LR{}_E_{}_ber_{}_lb_{}_BS_{}_step500_Learning_Curve.jpg".format(
                    dataset,
                    arch,
                    cfg.learning_rate,
                    cfg.epochs,
