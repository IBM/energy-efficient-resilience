...
class Self_Attention(nn.Module):
    def __init__(self):
        ...
        self.self_att = nn.Conv2d(
            in_channels=32, out_channels=3, kernel_size=1
        )
    ...
    def forward(self, image):
        ...
        g_x = self.query(x).view(
            batchsize, -1, N
        )  # Queries               [B, C_bar, N]
        h_x = self.value(x).view(
            batchsize, -1, N
        )  # Values                [B, C_bar, N]

        s = torch.bmm(
            f_x.permute(0, 2, 1), g_x
        )  # Scores                [B, N, N]
        ...
        v = v.view(
            batchsize, -1, H, W
        )  # Recover input shape   [B, C_bar, H, W]
