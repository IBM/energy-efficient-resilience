...
class nnLinearPerturbWeight(nn.Linear):
    ...
    def forward(self, input):
        if self.precision > 0:
            ...
            perturbed_weights = perturbweight(self.weight,self.precision,self.clamp_val, BitErrorMap0to1, BitErrorMap1to0)
...
class nnConv2dPerturbWeight(nn.Conv2d):
    ...
    def forward(self, input):
        if self.precision > 0:
            ...
            perturbweight = FaultInject.apply
            perturbed_weights = perturbweight(self.weight,self.precision,self.clamp_val, BitErrorMap0to1, BitErrorMap1to0)
        return F.conv2d(input, perturbed_weights, self.bias, self.stride, self.padding, self.dilation, self.groups)
