...
def transform_train(
    ...
    print(
        "========== Check setting: Epoch: {}, Batch_size: {}, N perturbed models: {}, Lambda: {}, BitErrorRate: {}, LR: {}==========".format(
            cfg.epochs, cfg.batch_size, cfg.N, lb, ber, cfg.learning_rate
        )
    )
    print(
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch_id, (image, label) in tqdm.tqdm(enumerate(trainloader)):
            ...
            for k in range(cfg.N):
                ...
                if cfg.layerwise:
                    ...
                    model, model_perturbed = model.to(
                        device
                    ), model_perturbed.to(device)
                ...
                if cfg.layerwise:
                    ...
                    for name, layer in model_np.named_modules():
                        if "relu" in name:
                            layer.register_forward_hook(
                                get_activation_c(activation_c, name)
                            )
                    ...
                    for name, layer in model_perturbed.named_modules():
                        if "relu" in name:
                            # print(name)
                            layer.register_forward_hook(
                                get_activation_p(activation_p, name)
                            )
                ...
                each_c_pred[k] += torch.sum(
                    pred_without_p == label.data
                ).item()
                ...
                running_correct_orig += torch.sum(
                    pred_orig == label.data
                ).item()
        ...
        print(
            "P 2-norm: {}, MSE: {}".format(
                torch.linalg.norm(torch.flatten(Pg.P), dim=0, ord=2),
                imgMSE(image, image_adv),
            )
        )
        print(
            "Each pred w/o transformation: {}".format(
                [x / len(trainloader.dataset) for x in each_c_pred]
            )
        )
        print(
            "Each pred with transformation: {}".format(
                [x / len(trainloader.dataset) for x in each_p_pred]
            )
        )

        # Keep the running accuracy of clean model and perturbed model for all mini-batch.
        accuracy_orig = running_correct_orig / (
            len(trainloader.dataset) * cfg.N
        )
    ...
    torch.save(
        {"Reprogrammed Perturbation": Pg.P},
        cfg.save_dir
        + "EOPM_arch_{}_LR_p{}_E_{}_ber_{}_lb_{}_Normal_initZero.pt".format(
            arch, cfg.learning_rate, cfg.epochs, ber, lb
        ),
    )
