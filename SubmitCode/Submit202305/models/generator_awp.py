...
class GeneratorConvLQ_AWP(nn.Module):
    """
    Apply reprogramming.
    """
    def __init__(self, precision):
        super(GeneratorConvLQ_AWP, self).__init__()
...
class GeneratorConvSQ_AWP(nn.Module):
    """
    Apply reprogramming.
    """
    def __init__(self, precision):
        super(GeneratorConvSQ_AWP, self).__init__()
...
class GeneratorDeConvLQ_AWP(nn.Module):
    """
    Apply reprogramming.
    """
    def __init__(self, precision):
        super(GeneratorDeConvLQ_AWP, self).__init__()
...
class GeneratorDeConvSQ_AWP(nn.Module):
    """
    Apply reprogramming.
    """
    def __init__(self, precision):
        super(GeneratorDeConvSQ_AWP, self).__init__()
...
class DoubleConvQ_AWP(nn.Module):
...
class DownQ_AWP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, precision):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConvQ_AWP(in_channels, out_channels, precision)
        )

    def forward(self, x):
        return self.maxpool_conv(x)

class UpQ_AWP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, precision, bilinear=True):
        ...
        if precision > 0:
            if bilinear:
                ...
                self.conv = DoubleConvQ_AWP(in_channels, out_channels, in_channels // 2, precision)
            else:
                ...
                self.conv = DoubleConvQ_AWP(in_channels, out_channels, precision)
        else:
            # if bilinear, use the normal convolutions to reduce the number of channels
            if bilinear:
                self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
...
class OutConvQ_AWP(nn.Module):
    def __init__(self, in_channels, out_channels, precision):
        super(OutConvQ_AWP, self).__init__()
        if precision > 0:
...
class GeneratorUNetLQ_AWP(nn.Module):
    def __init__(self, precision, bilinear=False):
        super(GeneratorUNetLQ_AWP, self).__init__()
        self.bilinear = bilinear

        self.inc = DoubleConvQ_AWP(3, 16, precision)
        self.down1 = DownQ_AWP(16, 32, precision)
        self.down2 = DownQ_AWP(32, 64, precision)
        factor = 2 if bilinear else 1
        self.down3 = DownQ_AWP(64, 128 // factor, precision)
        self.up1 = UpQ_AWP(128, 64 // factor, precision, bilinear)
        self.up2 = UpQ_AWP(64, 32 // factor, precision, bilinear)
        self.up3 = UpQ_AWP(32, 16 // factor, precision, bilinear)
        self.outc = OutConvQ_AWP(16, 3, precision)
...
class GeneratorUNetSQ_AWP(nn.Module):
    def __init__(self, precision, bilinear=False):
        super(GeneratorUNetSQ_AWP, self).__init__()
        self.bilinear = bilinear

        self.inc = DoubleConvQ_AWP(3, 8, precision)
        self.down1 = DownQ_AWP(8, 16, precision)
        self.down2 = DownQ_AWP(16, 32, precision)
        factor = 2 if bilinear else 1
        self.down3 = DownQ_AWP(32, 64 // factor, precision)
        self.up1 = UpQ_AWP(64, 32 // factor, precision, bilinear)
        self.up2 = UpQ_AWP(32, 16 // factor, precision, bilinear)
        self.up3 = UpQ_AWP(16, 8 // factor, precision, bilinear)
        self.outc = OutConvQ_AWP(8, 3, precision)
