...
def transform_train(
    ...
    print(
        "========== Check setting: Epoch: {}, Batch_size: {}, N perturbed models: {}, Lambda: {}, BitErrorRate: {}, LR: {}, G: {}==========".format(
            cfg.epochs,
            cfg.batch_size,
            cfg.N,
            lb,
    ...
    for epoch in range(cfg.epochs):
        ...
        print(
            "Each pred w/o transformation: {}".format(
                [
                    np.round(x / len(trainloader.dataset), decimals=4)
                    for x in each_c_pred
                ]
            )
        )
        print(
        ...
        if (epoch + 1) % 20 == 0 or (epoch + 1) == cfg.epochs:
            ...
            plt.savefig(
                cfg.save_dir
                + "EOPM_Generator{}Q_{}_arch_{}_LR{}_E_{}_ber_{}_lb_{}_N_{}_step500_LearningCurve.jpg".format(
                    cfg.G,
                    dataset,
                    arch,
                    cfg.learning_rate,
