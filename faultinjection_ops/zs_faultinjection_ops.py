...
class FaultInject(torch.autograd.Function):
    ...
    def forward(
        ...
        if (
            q_method == "symmetric_unsigned"
            or q_method == "asymmetric_unsigned"
        ):
            """
        ...
        if (
            ...
            input_qand = torch.bitwise_and(BitErrorMap1to0, input_q).to(
                torch.int8
            )
        ...
        input_dq_clamped = torch.clamp(
            input_dq, torch.min(input), torch.max(input)
        )
...
class nnLinearPerturbWeight(nn.Linear):
    ...
    def forward(self, input):
        ...
        if debug:
            print(
                torch.min(self.weight).detach().cpu().numpy(),
                torch.max(self.weight).detach().cpu().numpy(),
            )
            print(
                torch.min(perturbed_weights).detach().cpu().numpy(),
                torch.max(perturbed_weights).detach().cpu().numpy(),
            )
