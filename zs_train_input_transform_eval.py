...
def accuracy_checking_clean(model_orig, trainloader, testloader, device):
    ...
    print(
        "Accuracy of training data: clean model: {:5f}".format(
            accuracy_orig_train
        )
    )
    print(
        "Accuracy of testing data: clean model: {:5f}".format(
            accuracy_orig_test
        )
    )
...
def layerwise(nonzerolist, act_c):
    ...
    for name in layer_keys:
        nonzeros += torch.sum(
            (torch.nn.Tanh()(act_c[name]) != 0).int(), [1, 2, 3]
        ).data
        alls += torch.sum(
            (torch.nn.Tanh()(act_c[name]) >= 0).int(), [1, 2, 3]
        ).data
...
def transform_eval(
    ...
    if cfg.testing_mode == "clean":
        model, checkpoint_epoch = init_models(
            arch, 3, precision, True, checkpoint_path, dataset
        )
    ...
    if cfg.testing_mode == "random_bit_error":
        ...
        print(
            "========== Start checking the accuracy with different perturbed model: bit error mode =========="
        )
        ...
        for i in range(50000, 50010):
            ...
            accuracy_orig_test_list_with_transformation.append(
                accuracy_orig_test
            )
    ...
    if cfg.testing_mode == "activation":
        ...
        print(
            "Training data => Accuracy w/o transformation: {}, The ratio of the nonzero activation: {:.5f}".format(
                accuracy_orig_train, nonzeroActRation
            )
        )
        ...
        print(
            "Testing data => Accuracy w/o transformation: {}, The ratio of the zero activation: {:.5f}".format(
                accuracy_orig_test, nonzeroActRation
            )
        )
        ...
        nonzeroActRation = np.round_(meanActivation, decimals=7)
        print(
            "Training data => Accuracy with transformation: {}, The ratio of the zero activation: {:.5f}".format(
                accuracy_orig_train, nonzeroActRation
            )
        )
        ...
        nonzeroActRation = np.round_(meanActivation, decimals=5)
        print(
            "Testing data => Accuracy with transformation: {}, The ratio of the zero activation: {:.5f}".format(
                accuracy_orig_test, nonzeroActRation
            )
        )
