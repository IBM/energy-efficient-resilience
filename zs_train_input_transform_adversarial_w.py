...
def draw_learning_curve(loss_list, epoch, arch, ber, lb):
    ...
    plt.savefig(
        cfg.save_dir_curve
        + 'adversarial_arch_{}_LR_a{}_p{}_E_{}_PGD_{}_ber_{}_lb_{}.png'.format(arch, cfg.learning_rate_adversarial, cfg.learning_rate, cfg.epochs, cfg.PGD_STEP, ber, lb)
    )
...
def transform_train(
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch_id, (image, label) in enumerate(trainloader):
            ...
            model_perturbed = copy.deepcopy(model) # Because we can't modify the origin model weights, therefore, copy a new model to apply ADV attack.
            model.eval()
            ...
            for param_orig, param_p in zip(model.parameters(), model_perturbed.parameters()):
    ...
    for i in range(50000, 50010):
        ...
        (_, _, model_rbe, _) = init_models_pairs(arch, in_channels, precision, True, 
            checkpoint_path, fl,  ber, pos, seed=i) 
        ...
        model, model_rbe, model_rbe_robust = model.to(device), model_rbe.to(device), model_rbe_robust.to(device)
        model.eval()
        model_rbe.eval()
        ...
        accuracy_orig_train, accuracy_p_train, accuracy_orig_test, accuracy_p_test = accuracy_checking(model, model_rbe, trainloader, testloader, Pg, device, use_transform=False)

        # Roubstness model with bit error
        print('Testing robust bit error model: ')
        accuracy_orig_train, accuracy_p_train, accuracy_orig_test, accuracy_p_test = accuracy_checking(model, model_rbe_robust, trainloader, testloader, Pg, device, use_transform=False)