...
def collect_activation_sizes(self, input, output):
    global layer_counter, input_w, input_h, input_c, filter_h, filter_w, filter_c, m_strides
    if 'Conv2d' in self.__class__.__name__ or 'Transpose2d' in self.__class__.__name__:
        ...
        if 'Conv2d' in self.__class__.__name__:
            ...
            filter_c[layer_counter] = f_shape[0] # Different from DeConv2d
            m_strides[layer_counter] = 1
        else:
            input_w[layer_counter] = i_shape[3]
            input_h[layer_counter] = i_shape[2]
            input_c[layer_counter] = i_shape[1]
            filter_w[layer_counter] = f_shape[3]
            filter_h[layer_counter] = f_shape[2]
            filter_c[layer_counter] = f_shape[1] # Different from Conv2d
            m_strides[layer_counter] = 2
        layer_counter +=1
...
def main():
    ...
    in_channels = 3
    classes = 10
    model = generator.GeneratorConvLQ(precision = 8)
    model = generator.GeneratorConvSQ(precision = 8)
    model = generator.GeneratorDeConvLQ(precision = 8)
    model = generator.GeneratorDeConvSQ(precision = 8)
    model = generator.GeneratorUNetLQ(precision = 8)
